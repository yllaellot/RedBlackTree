...
template<class T, class CMP>
inline int64_t RedBlackTree<T, CMP>::range_queries_2_search(const T& key1, const T& key2)
{
        Node* node = root;
        Node* result = nullptr;
        int64_t last_r = 0;
        ...
        while (node != TNULL)
        {
            if (!CMP{}(node->data, key1)) // key <= node->key()
            {
                num_elems -= last_r;
                last_r = 0;
                //std::cout<<"num "<<num_elems<<" curr nodel "<< node->data <<std::endl;
                result = std::__exchange(node, node->left);
            }
            else
            {
                is_min = false;
                //std::cout<<"num "<<num_elems<<" curr noder "<< node->data <<std::endl;
                //std::cout<<"last_r "<<last_r <<std::endl;
                last_r += node->cnt + 1;
                //std::cout<<"last_r after "<<last_r <<std::endl;
                node = node->right;
            }
        }

        if (result == nullptr) return 0;
        // if(result->data > key){num_elems += result->cnt;}
        // else
        //  {
        //     num_elems += result->cnt + 1;
        //  }
        if (is_min && result->data != key1) num_elems = 0;
        else
        {
    
        if (!CMP{}(key1, result->data)) num_elems -= 1;

        //if (result != TNULL) num_elems -= result->cnt;
        num_elems -= result->cnt;
        }
        //std::cout<<"num cnt "<<result->cnt <<std::endl;

        //std::cout<<"num "<<num_elems <<std::endl;

        //std::cout<<"res "<< result->data <<std::endl;
        ...
        while (node != TNULL)
        {
            if (!CMP{}(node->data, key2)) // key <= node->key()
            {
                num_elems += last_r;
                last_r = 0;
                //std::cout<<"num "<<num_elems<<" curr nodel "<< node->data <<std::endl;
        ...
        else
        {
            ...
            {
                if (CMP{}( result->data, key2))num_elems += 1;
...
template<class T, class CMP>
void RedBlackTree<T, CMP>::dump2(Node* node, std::string const & rpref, std::string const & cpref, std::string const & lpref) {
    if (!node) return;
    if (node->right)
...
template<class T, class CMP>
inline int64_t RedBlackTree<T, CMP>::range_queries_2(const T& key1, const T& key2)
{
    if (key2 <= key1) return 0;

    return range_queries_2_search(key1, key2);
}
